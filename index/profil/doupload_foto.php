...
if($_POST['id_user']!="" && $_FILES['foto_user']['name']!="")
{
		...
		else
		{
			...
			{
				$cek_file_foto = mysqli_query($db_dash,"SELECT * FROM user where id_user='".$_POST['id_user']."'");
				//$num_file_kendaraan = mysqli_num_rows($cek_file_foto);
			
				$upload_file_foto = mysqli_query($db_dash,"UPDATE user SET user_foto = '".$nama_foto_baru."' where id_user = '".$_POST['id_user']."'");	
